...
\section{Introduction}
  ...
  The most important content parameter is called relevance. However, this parameter contains somehow inherently other parameters one of which we could call .
  ...
  E.g. we can say that the document written solely in some language $L_a$ would not be readable for a reader that does not know language $L_a$. 
  In this case the so called "major" language of the page is $L_a$, or we can write that  $L_{maj} = L_a$.
  ...
  This definition implies that if a user searches for a webpages in a language $L_a$, our major goal is to provide webpages for which $L_{maj} = L_a$.
...
\section{Languages}
 ...
 \end{table}
  ...
  The RC-Rank algorithm is suitable for both classification and ranking. 
