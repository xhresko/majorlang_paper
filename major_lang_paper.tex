...
\section{Introduction}
  Delivering the content based on the preferences of the user is one of the basic tasks in the area of information retrieval.
  The most important content parameters is called relevance. However, this parameter contains somehow inherently the parameter of usability.
  This hidden parameter could be defined as a probability, that user will be able to use the content of particular document to satisfy his (intended) needs.
  Usability of the document is not solely a parameter of a document. We could say that it is a parameter of the user - document relation .

  Language of the document affects the usability with different intensity. The weight of its influence depends on the type of document content.
  ...
  Document readability is telling us if particular user is able to "use" texts on the web page for his intent (e.g. reading a joke, comments or navigating menus).
  
  The readability could be easily measured if we know that the page contains texts written in only one language and if we know the languages known by the user.
  E.g. we can say that the document written solely in some language $L_x$ would not be readable for a reader that does not know language $L_x$. 
  In this case the so called "major" language of the page is $L_x$, or we can write that  $L_{maj} = L_x$.
  ...
  For these it could be harder to decide which language would be the "major".
  %However, determining which of them is major one is not.
  ...
  This definition implies that if a user searches for a webpages in a language $L_x$, our major goal is to provide webpages for which $L_{maj} = L_x$.
  
  For each webpage, we thus need to ask: “If the user wanted to find this page, in which language would she formulate the query?” 
  Our interpretation of major language presupposes that user knows what should be the language of the answer to her query.
...
\section{Languages}
 ...
 \end{table}
  ...
  presence of language L leads to classifying an example as L. Therefore, we decided to approach our task as binary 
  ...
  For each language L, our binary classifier retrieves generic language features of the language L and all language-specicific
  ones and says, whether the given example is in language L or not. 
...
\subsubsection{Language-specific features}
  ...
  \begin{itemize}
    \item \textit{on-page language detection} - 
      ...
      contained on the page with on-page detector    CITACE REHUREK     . 
 ...
 \section{Conclusions}
 Seznam.cz used to detect the major language of a website by hand crafted algorhitm
 employing a decision tree implemented as if conditions. Applying machine learning
 to this task brought Seznam.cz significant improvement in precision and recall.
 
 To make current algorhitm perform even better, we see an opportunity in web page type
 classifier. If this worked with higher precision, it would be possible to set aside
 difficult web pages such as online marketplaces and the learning algorhitm would be able to
 use different features.
 The above desribed approach towards major language detection has been implemented
